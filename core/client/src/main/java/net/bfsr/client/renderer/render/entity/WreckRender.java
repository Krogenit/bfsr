...
public class WreckRender extends Render<Wreck> {
    @Getter
    private final Texture textureFire, textureLight;
    ...
    public WreckRender(Wreck object) {
        super(TextureLoader.getTexture(object.getWreckData().getTexture()), object, 0.5f, 0.5f, 0.5f, 1.0f);
    ...
    public void renderAlpha() {
        ...
        SpriteRenderer.get().addToRenderPipeLineSinCos(lastPosition.x, lastPosition.y, position.x, position.y, lastSin, lastCos, object.getSin(), object.getCos(),
                size.x, size.y, color.x, color.y, color.z, color.w, texture, BufferType.ENTITIES_ALPHA);
    }
    ...
    public void renderAdditive() {
        if (colorFire.w > 0) {
            ...
            Vector2f size = object.getSize();
            SpriteRenderer.get().addToRenderPipeLineSinCos(lastPosition.x, lastPosition.y, position.x, position.y, lastSin, lastCos, object.getSin(), object.getCos(),
                    size.x, size.y, lastColorFire, colorFire, textureFire, BufferType.ENTITIES_ADDITIVE);
        }
        ...
        if (colorLight.w > 0) {
            ...
            Vector2f size = object.getSize();
            SpriteRenderer.get().addToRenderPipeLineSinCos(lastPosition.x, lastPosition.y, position.x, position.y, lastSin, lastCos, object.getSin(), object.getCos(),
                    size.x, size.y, lastColorLight, colorLight, textureLight, BufferType.ENTITIES_ADDITIVE);
        }
    }
}