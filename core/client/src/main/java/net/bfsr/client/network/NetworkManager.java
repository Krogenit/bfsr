...
public abstract class NetworkManager extends SimpleChannelInboundHandler<Packet> {
    ...
    protected final Queue<PacketIn> receivedPacketsQueue = Queues.newConcurrentLinkedQueue();
    ...
    protected void channelRead0(ChannelHandlerContext channelHandlerContext, Packet packet) throws IOException {
        if (this.channel.isOpen()) {
            if (packet.hasPriority()) {
                processPacket((PacketIn) packet);
            } else {
                this.receivedPacketsQueue.add((PacketIn) packet);
            }
        }
    }
    ...
    public void scheduleOutboundPacket(Packet packet, GenericFutureListener<?>... listeners) {
        if (this.channel != null && this.channel.isOpen()) {
            ...
            this.outboundPacketsQueue.add(new InboundHandlerTuplePacketListener(packet, listeners));
        }
    }
    ...
    public void processReceivedPackets() {
        ...
        for (int i = 1000; !this.receivedPacketsQueue.isEmpty() && i >= 0; --i) {
            PacketIn packet = this.receivedPacketsQueue.poll();
    ...
    protected abstract void processPacket(PacketIn packet);
