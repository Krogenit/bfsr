...
import net.bfsr.client.renderer.render.component.WeaponSlotBeamRender;
import net.bfsr.client.renderer.render.component.WeaponSlotRender;
...
public class ShipRender extends Render<Ship> {
    ...
    public ShipRender(Ship ship) {
        super(TextureLoader.getTexture(ship.getShipData().getTexture()), ship);
        textureDamage = TextureLoader.getTexture(ship.getShipData().getDamageTexture());
        ...
        this.shieldTexture = TextureLoader.getTexture(ship.getShield().getShieldData().getTexturePath());
    ...
    private void spawnEngineParticles(Direction direction) {
        ...
        if (direction == Direction.STOP) {
            ...
            if (Math.abs(x) > 10) {
                spawnEngineParticles(MathUtils.calculateDirectionToOtherObject(object, x + shipPos.x, shipPos.y));
            }

            if (Math.abs(y) > 10) {
                spawnEngineParticles(MathUtils.calculateDirectionToOtherObject(object, shipPos.x, y + shipPos.y));
            }
    ...
    public void renderAlpha() {
        if (object.isSpawned()) {
            ...
            SpriteRenderer.get().addToRenderPipeLineSinCos(lastPosition.x, lastPosition.y, position.x, position.y, lastSin, lastCos, sin, cos, scale.x, scale.y,
                    1.0f, 1.0f, 1.0f, 1.0f, texture, maskTexture, BufferType.ENTITIES_ALPHA);
    ...
    public void renderAdditive() {
        if (object.isSpawned()) {
            ...
            float delta = lastJumpDelta + (jumpDelta - lastJumpDelta) * Core.get().getRenderer().getInterpolation();
            float size = 40.0f * delta;
            Vector4f effectsColor = object.getShipData().getEffectsColor();

            SpriteRenderer.get().addToRenderPipeLineSinCos(lastJumpPosition.x, lastJumpPosition.y, jumpPosition.x, jumpPosition.y, object.getSin(),
                    object.getCos(), size, size, effectsColor.x, effectsColor.y, effectsColor.z, delta, JUMP_TEXTURE, BufferType.ENTITIES_ADDITIVE);
        }
    }
    ...
    private void renderShield(Ship ship, Shield shield) {
        if (shield != null && shield.isShieldAlive()) {
            ...
            SpriteRenderer.get().addToRenderPipeLineSinCos(lastPosition.x, lastPosition.y, ship.getPosition().x, ship.getPosition().y, lastSin, lastCos,
                    ship.getSin(), ship.getCos(), diameter.x * size, diameter.y * size, color.x, color.y, color.z, color.w, shieldTexture,
    ...
    public Texture getWeaponSlotTexture(int id) {
