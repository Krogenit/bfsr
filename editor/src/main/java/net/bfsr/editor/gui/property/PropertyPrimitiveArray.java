...
public class PropertyPrimitiveArray<V extends PropertiesHolder, PRIMITIVE_TYPE>
        extends PropertyArray<V, PropertyComponent<?>, PRIMITIVE_TYPE> {
    private final PropertyGuiElementType propertyGuiElementType;
    private final String propertyName;
    private final List<PRIMITIVE_TYPE> objects = new ArrayList<>();

    public PropertyPrimitiveArray(int width, int height, String name, FontType fontType, int fontSize, int propertyOffsetX,
                                  int stringOffsetY, Supplier<PRIMITIVE_TYPE> supplier,
                                  V object, List<Field> fields, Object[] values, PropertyGuiElementType propertyGuiElementType,
                                  String propertyName) {
    ...
    public PropertyComponent<V> add(PRIMITIVE_TYPE arrayElement) {
        try {
            PropertyComponent<V> propertyComponent =
                    ComponentBuilder.build(propertyGuiElementType, baseWidth - propertyOffsetX, baseHeight, propertyName,
                            fontType.getStringCache().getStringWidth(propertyName, fontSize), fontType,
                            fontSize, stringOffsetY, fields, new Object[]{arrayElement}, (Consumer<PRIMITIVE_TYPE>) objects::add,
