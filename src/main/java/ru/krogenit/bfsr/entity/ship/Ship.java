...
import ru.krogenit.bfsr.ai.Ai;
import ru.krogenit.bfsr.ai.AiAggressiveType;
import ru.krogenit.bfsr.ai.task.AiAttackTarget;
import ru.krogenit.bfsr.ai.task.AiSearchTarget;
...
import ru.krogenit.bfsr.network.packet.common.PacketObjectPosition;
...
//		this.ai.addTask(new AiAttackTarget(this, 1000f));
	...
	public void control(double delta) {
		if(destroingTimer == 0) {
			if(body.isAsleep()) body.setAsleep(false);
...
//				}
	...
	public void checkCollision(ContactPoint contact, Body body) {
		...
		if(userData != null) {
			if(userData instanceof Ship) {
				...
				if(impactPowerForOther > 0.25f) otherShip.damageByCollision(this, impactPowerForOther, contact);
				...
				if(collisionTimer <= 0) {
					...
					if(world.isRemote()) {
						if(shield != null) {
							...
							ParticleSpawner.spawnDirectedSpark(contact, 45f, new Vector4f(color));
						} else {
							ParticleSpawner.spawnDirectedSpark(contact, 37.5f, new Vector4f(1f,1f,1f,1f));
						}
					}
				}
			}
	...
	public boolean damageByCollision(Ship otherShip, float impactPower, ContactPoint contact) {
		if(collisionTimer > 0) {
			return false;
		}
