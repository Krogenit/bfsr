package ru.krogenit.bfsr.component.weapon;
...
import ru.krogenit.bfsr.network.packet.common.PacketWeaponShoot;
...
public abstract class WeaponSlotBeam extends WeaponSlot {
	...
	private final List<RaycastResult> result = new ArrayList<>();
	...
	private void rayCast(double delta) {
		World physicWorld = world.getPhysicWorld();
		
		float cos = ship.getCos();
		float sin = ship.getSin();
		float startRange = -scale.x / 2f;
		...
		physicWorld.raycast(start, end, filter, true, true, false, result);
		if(result.size() > 0) {
			RaycastResult res = result.get(0);
			...
			currentBeamRange = (float) start.distance(point);
