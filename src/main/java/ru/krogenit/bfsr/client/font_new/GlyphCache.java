...
public class GlyphCache {
    ...
    int getStringHeight(String s) {
        ...
        LineMetrics lineMetrics = font.getLineMetrics(s, this.fontRenderContext);
        return (int) lineMetrics.getHeight() / 2;
    }

    public int getStringAscent(String s) {
        Font font = lookupFont(s.toCharArray(), 0, s.length(), Font.PLAIN);
        LineMetrics lineMetrics = font.getLineMetrics(s, this.fontRenderContext);
    ...
    private void updateTexture(Rectangle dirty) {
        ...
        if (dirty != null) {
            ...
            OpenGLHelper.bindTexture(textureName);
    ...
    private void allocateGlyphCacheTexture() {
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, antiAliasEnabled ? GL_ALPHA8 : GL_ALPHA4, TEXTURE_WIDTH, TEXTURE_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, imageBuffer);
