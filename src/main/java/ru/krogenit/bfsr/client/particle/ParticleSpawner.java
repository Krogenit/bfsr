...
import ru.krogenit.bfsr.network.packet.server.PacketSpawnParticle;
...
//		rot += Math.PI / 2.0;
    ...
    public static void spawnDirectedSpark(ContactPoint contact, float size, Vector4f color) {
    ...
    public static void spawnDirectedSplat(ContactPoint contact, float size, Vector4f color) {
        Vector2 point = contact.getPoint();
        Vector2 normal = contact.getNormal();
        Vector2f pos = new Vector2f((float) point.x, (float) point.y);
        float rot = (float) Math.atan2(normal.x, -normal.y);
