...
import ru.krogenit.bfsr.client.shader.NebulaShader;
...
public class TextureGenerator {
	...
	public static Texture generateNoiseTexture(Random random, int size) {
		...
		OpenGLHelper.bindTexture(texture.getId());
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RG8, size, size, 0, GL_RG, GL_UNSIGNED_BYTE, buffer);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
...
//		lastTexture = activeBuffer;
		...
		glViewport(0, 0, Core.getCore().getWidth(), Core.getCore().getHeight());
		OpenGLHelper.activateTexture0();
