...
public final class Truetype extends FontDemo {
    ...
    private Truetype(String filePath) {
        ...
        try {
            byte[] bytes = Files.readAllBytes(PathHelper.FONT.resolve(filePath));
    ...
    public static void main(String[] args) {
        ...
        if (args.length == 0) {
            System.out.println(
                    "Use 'ant demo -Dclass=org.lwjgl.demo.stb.Truetype -Dargs=<path>' to load a different text file (must be UTF8-encoded).\n");
            filePath = "doc/README.md";
        } else {
            filePath = args[0];
        }

        new Truetype("Xolonium-Regular.ttf").run("STB Truetype Demo");
    }
