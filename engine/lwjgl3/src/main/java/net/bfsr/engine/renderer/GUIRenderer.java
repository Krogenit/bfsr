...
public final class GUIRenderer extends AbstractGUIRenderer {
    ...
    @Override
    public void addCentered(float x, float y, float sizeX, float sizeY, float r, float g, float b, float a,
    ...
    @Override
    public void add(float lastX, float lastY, float x, float y, float lastRotation, float rotation, float sizeX, float sizeY,
    ...
    @Override
    public void add(float lastX, float lastY, float x, float y, float lastSin, float lastCos, float sin, float cos, float sizeX,
                    float sizeY, float r, float g, float b, float a, AbstractTexture texture) {
        add(lastX, lastY, x, y, lastSin, lastCos, sin, cos, sizeX, sizeY, r, g, b, a, texture.getTextureHandle());
    }

    public void add(float x, float y, float rotation, float sizeX, float sizeY, float r, float g, float b, float a,
                    long textureHandle) {
        spriteRenderer.putVerticesCenteredClockWise(x, y, LUT.sin(rotation), LUT.cos(rotation), sizeX * 0.5f, sizeY * 0.5f,
    ...
    public void add(float lastX, float lastY, float x, float y, float lastSin, float lastCos, float sin, float cos, float sizeX,
                    float sizeY, float r, float g, float b, float a, long textureHandle) {
        ...
        spriteRenderer.putVerticesCenteredClockWise(lastX + (x - lastX) * interpolation, lastY + (y - lastY) * interpolation,
                lastSin + (sin - lastSin) * interpolation, lastCos + (cos - lastCos) * interpolation, sizeX * 0.5f, sizeY * 0.5f,
                buffersHolder.getVertexBuffer(), buffersHolder.getVertexBufferIndex());
        spriteRenderer.putColor(r, g, b, a, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putTextureHandle(textureHandle, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putMaterialData(0, 0.0f, 0.0f, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
    ...
    public void add(float lastX, float lastY, float x, float y, float lastRotation, float rotation, float sizeX, float sizeY,
        ...
        spriteRenderer.putVerticesCenteredClockWise(lastX + (x - lastX) * interpolation, lastY + (y - lastY) * interpolation, sin,
                cos, sizeX * 0.5f, sizeY * 0.5f, buffersHolder.getVertexBuffer(), buffersHolder.getVertexBufferIndex());
        spriteRenderer.putColor(r, g, b, a, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putTextureHandle(textureHandle, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putMaterialData(0, 0.0f, 0.0f, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        buffersHolder.incrementObjectCount();
    ...
    public void add(float x, float y, float sizeX, float sizeY, float r, float g, float b, float a, long textureHandle) {
        spriteRenderer.putVerticesClockWise(x, sizeY + y, sizeX + x, sizeY + y, sizeX + x, y, x, y,
                buffersHolder.getVertexBuffer(),
    ...
    public void addCentered(float x, float y, float sizeX, float sizeY, float r, float g, float b, float a, long textureHandle) {
        spriteRenderer.putVerticesCenteredClockWise(x, y, sizeX * 0.5f, sizeY * 0.5f, buffersHolder.getVertexBuffer(),
                buffersHolder.getVertexBufferIndex());
        spriteRenderer.putColor(r, g, b, a, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putTextureHandle(textureHandle, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putMaterialData(0, 0.0f, 0.0f, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
    ...
    public void add(float lastX, float lastY, float x, float y, float sizeX, float sizeY, float r, float g, float b, float a,
        ...
        spriteRenderer.putVerticesClockWise(interpolatedX, sizeY + interpolatedY, sizeX + interpolatedX, sizeY + interpolatedY,
                sizeX + interpolatedX, interpolatedY, interpolatedX, interpolatedY, buffersHolder.getVertexBuffer(),
                buffersHolder.getVertexBufferIndex());
        spriteRenderer.putColor(r, g, b, a, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putTextureHandle(textureHandle, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
        spriteRenderer.putMaterialData(0, 0.0f, 0.0f, buffersHolder.getMaterialBuffer(), buffersHolder.getMaterialBufferIndex());
