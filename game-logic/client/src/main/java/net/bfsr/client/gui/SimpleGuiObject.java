...
public class SimpleGuiObject extends AbstractGuiObject {
    ...
    public void render() {
        if (rotation != 0.0f) {
            ...
            if (isMouseHover()) {
                guiRenderer.add(lastX + 1, lastY + 1, x + 1, y + 1, lastRotation, rotation, width - 2, height - 2, hoverColor.x, hoverColor.y, hoverColor.z, hoverColor.w);
            } else {
                guiRenderer.add(lastX + 1, lastY + 1, x + 1, y + 1, lastRotation, rotation, width - 2, height - 2, color.x, color.y, color.z, color.w);
            }
        } else {
            if (isMouseHover()) {
                guiRenderer.add(lastX, lastY, x, y, width, height, outlineHoverColor.x, outlineHoverColor.y, outlineHoverColor.z, outlineHoverColor.w);
                guiRenderer.add(lastX + 1, lastY + 1, x + 1, y + 1, width - 2, height - 2, hoverColor.x, hoverColor.y, hoverColor.z, hoverColor.w);
            } else {
                guiRenderer.add(lastX, lastY, x, y, width, height, outlineColor.x, outlineColor.y, outlineColor.z, outlineColor.w);
    ...
    public void renderNoInterpolation() {
        if (isMouseHover()) {
            guiRenderer.add(x, y, width, height, outlineHoverColor.x, outlineHoverColor.y, outlineHoverColor.z, outlineHoverColor.w);
