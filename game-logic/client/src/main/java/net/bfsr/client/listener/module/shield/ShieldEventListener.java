...
public class ShieldEventListener {
    ...
    public void event(ShieldRebuildEvent event) {
        ...
        ShieldEffects.rebuild(position.x, position.y, ship.getSize().x * 2.0f, shipEffectColor.x, shipEffectColor.y, shipEffectColor.z, 1.0f);
    }
    ...
    public void event(ShieldRemoveEvent event) {
        ...
        Vector2f position = ship.getPosition();
        Vector4f shipEffectColor = ship.getShipData().getEffectsColor();
        ShieldEffects.disable(position.x, position.y, ship.getSize().x * 2.0f, shipEffectColor.x, shipEffectColor.y, shipEffectColor.z, 1.0f);
    }

    @Handler
    public void event(ShieldDamageByCollision event) {
        Vector4f color = event.ship().getShipData().getEffectsColor();
