...
public class GarbageSpawner {
    ...
    public void bulletArmorDamage(float x, float y, float z, float velocityX, float velocityY, float normalX, float normalY) {
        smallGarbage(1 + random.nextInt(3), x, y, z, velocityX + normalX,
                velocityY + normalY, RandomHelper.randomFloat(random, 0.055f, 0.165f), 0.3f, 0.5f);
    }

    public void bulletHullDamage(float x, float y, float z, float velocityX, float velocityY, float normalX, float normalY) {
        if (random.nextInt(2) == 0) {
            spawnShipOst(x, y, z, velocityX + normalX * (random.nextFloat() * 0.5f + 0.5f),
    ...
    public void beamHullDamage(float x, float y, float z, float velocityX, float velocityY) {
        if (random.nextInt(5) == 0) {
            if (random.nextInt(50) == 0) {
                RotationHelper.angleToVelocity(MathUtils.TWO_PI * random.nextFloat(), 0.15f, cachedVector);
                spawnShipOst(x, y, z, velocityX + cachedVector.x, velocityY + cachedVector.y);
            }
    ...
    public void spawnShipOst(float x, float y, float z, float velocityX, float velocityY) {
        RotationHelper.angleToVelocity(MathUtils.TWO_PI * random.nextFloat(), 0.02f + random.nextFloat() * 0.2f, cachedVector);
        ost.play(x, y, z, 0, 0, velocityX + cachedVector.x, velocityY + cachedVector.y);
    }

    public void smallGarbage(int count, float x, float y, float z, float velocityX, float velocityY, float size, float sizeVel,
                             float alphaVel) {
        smallGarbage(count, x, y, z, velocityX, velocityY, () -> cachedVector.set(0), size, sizeVel, alphaVel);
    }

    public void smallGarbage(int count, float x, float y, float z, float velocityX, float velocityY, float size) {
        smallGarbage(count, x, y, z, velocityX, velocityY, () -> {
            RotationHelper.angleToVelocity(MathUtils.TWO_PI * random.nextFloat(), 0.02f + random.nextFloat() * 0.2f, cachedVector);
            return cachedVector;
        }, size, 0.05f, 0.12f);
    }
    ...
    public void smallGarbage(int count, float x, float y, float z, float velocityX, float velocityY,
        ...
        for (int i = 0; i < count; i++) {
            ...
            particleManager.createParticle().init(TextureRegister.particleGarbage0, x, y, z, velocityX + localVelocity.x,
                    velocityY + localVelocity.y, LUT.sin(angle), LUT.cos(angle), angularVelocity, size, size, sizeVel, 0.6f, 0.6f, 0.6f,
                    1.0f, alphaVel, false, ParticleType.ALPHA_BLENDED);
        }
    }
}