...
public class PacketSyncDamageHandler extends PacketHandler<PacketSyncDamage, NetworkSystem> {
    @Override
    public void handle(PacketSyncDamage packet, NetworkSystem networkSystem, ChannelHandlerContext ctx, InetSocketAddress remoteAddress) {
        ...
        if (gameObject instanceof Damageable damageable) {
            ...
            DamageHandler.updateDamage(damageable, packet.getX(), packet.getY(), packet.getWidth(), packet.getHeight(), packet.getByteBuffer());
        }
    }
}