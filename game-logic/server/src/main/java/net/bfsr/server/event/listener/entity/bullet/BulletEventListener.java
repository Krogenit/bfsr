...
public class BulletEventListener {
    ...
    public void event(BulletAddToWorldEvent event) {
        ...
        ServerGameLogic.getNetwork().sendUDPPacketToAllNearby(new PacketSpawnBullet(bullet), bullet.getPosition(), WorldServer.PACKET_SPAWN_DISTANCE);
    }

    @Handler
    public void event(BulletReflectEvent event) {
        Bullet bullet = event.bullet();
        ServerGameLogic.getNetwork().sendUDPPacketToAllNearby(new PacketObjectPosition(bullet), bullet.getPosition(), WorldServer.PACKET_UPDATE_DISTANCE);
    }
    ...
    public void event(BulletDamageShipHullEvent event) {
        ...
        if (hull.getHull() / hull.getMaxHull() < 0.25f && rand.nextInt(2) == 0) {
            RotationHelper.angleToVelocity(net.bfsr.engine.math.MathUtils.TWO_PI * rand.nextFloat(), 1.5f, MathUtils.ANGLE_TO_VELOCITY);
